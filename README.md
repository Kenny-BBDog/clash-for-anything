...
## ⛓️ 特色功能：永久订阅桥 (Sub Bridge)

**解决订阅转换 502 报错的终极方案。** 该脚本在本地直接将 3x-ui 链接转换为 Clash 配置，无需请求 GitHub 或第三方转换接口。

1. **设置**：
   修改 `clash-converter/sub_bridge.py` 中的 `SOURCE_SUBS` 为你 3x-ui 的原始链接。
2. **运行**：
   ```bash
   python3 clash-converter/sub_bridge.py
   ```
3. **订阅**：
   在 Clash 中添加订阅：`http://你的VPS_IP:8080/my-stable-sub`。

---

## 🎟️ 特色功能：访客通行证 (Guest Pass)

**遇到不想长期分享节点的情况？** 使用访客通行证可以生成一个**限时 30 分钟**的临时 Shadowsocks 节点。

1. 在 VPS 终端运行：
```bash
python3 /etc/x-ui/guest_pass.py
```
*(注：建议将脚本放在易记位置，脚本会自动在 3x-ui 数据库中插入限时节点并打印 ss:// 链接)*

2. **特点**：
- ✅ **自动销毁**：30 分钟到期后系统自动禁用，无需手动干预。
- ✅ **全兼容性**：Shadowsocks 协议支持几乎所有设备（包括非 Clash 终端）。
- ✅ **安全可控**：不影响你原本的节点配置。
...
### v3.3.0 (2026-02-02)
- ⛓️ **新增永久订阅桥 (Sub Bridge)**：本地化转换逻辑，彻底解决 502 报错，支持 3x-ui 节点变更一键同步。
- 📦 **本地化增强**：支持读取本地模板，不再依赖 GitHub 网络环境。

### v3.2.0 (2026-02-02)
- 🎟️ **新增访客通行证 (Guest Pass)**：支持一键生成限时 30 分钟的临时 SS 节点。
- 🛡️ **安全增强**：针对非授权长期使用的场景提供自动化隔离方案。
